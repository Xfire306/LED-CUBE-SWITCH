...
int on_1; int on_2; //this is for random setting HIGH or LOW for the voltage layers int on_3; int on_4;

int ground_1; //same for ground int ground_2; int ground_3; int ground_4; int ground_5; int ground_6; int ground_7; int ground_8; int ground_9; int ground_10; int ground_11; int ground_12;

int xPin = A1; //Joystick Pins for inputs int yPin = A0;

int buttonPin = 0;

int Layer_1 = A2; //Voltage Layers for the cathode parts. We'll use analog so we can reserve digital ports for grounds int Layer_2 = A3; int Layer_3 = A4; int Layer_4 = A5;

int Bpin_1 = 2; //ground for pattern 1: middle square int Gpin_1 = 3; int Rpin_1 = 4;

int Bpin_2 = 5; //ground for pattern 2: diamond 1 int Gpin_2 = 6; int Rpin_2 = 7;

int Bpin_3 = 8; //ground for pattern 2: diamond 2 int Gpin_3 = 9; int Rpin_3 = 10;

int Bpin_4 = 11; //ground for pattern 2: diamond 2 int Gpin_4 = 12; int Rpin_4 = 13;

int xPosition = 0; int yPosition = 0; int buttonState = 0;

void setup() { // initialize serial communications at 9600 bps: Serial.begin(9600);

//Pins for the Joystick pinMode(xPin, INPUT); pinMode(yPin, INPUT);

//Pinmodes for Cube Layers and Grounds pinMode(Layer_1, OUTPUT); pinMode(Layer_2, OUTPUT); pinMode(Layer_3, OUTPUT); pinMode(Layer_4, OUTPUT);
...
// For versions prior to Arduino 1.0.1 // pinMode(buttonPin, INPUT); // digitalWrite(buttonPin, HIGH);

}

void loop() { xPosition = analogRead(xPin); yPosition = analogRead(yPin); buttonState = digitalRead(buttonPin);
...
if(xPosition > 1000) //Pattern for:::: { } if(xPosition < 400) //Pattern for::::; { } if(yPosition < 400) // Pattern for::: { } if(yPosition > 1000) // Pattern for::: { }

if(buttonState < 1) //Pattern for random flickerings. Will end with a random result { while(randEnd < 50) { on_1 = random(1,100);if(on_1 <50){on_1 = 200;} else {on_1 = 0;} //randomizes which Voltage layer will be on on_2 = random(1,100);if(on_2 <50){on_1 = 200;} else {on_2 = 0;} on_3 = random(1,100);if(on_3 <50){on_3 = 200;} else {on_3 = 0;} on_4 = random(1,100);if(on_4 <50){on_4 = 200;} else {on_4 = 0;}
